...
class Snapshot(object):
    ...
    def _do_scene_operation(self, operation, path=None, result_type=None):
        ...
        try:
            result = self._app.execute_hook(
                "hook_scene_operation", operation=operation, file_path=path
            )
        ...
        if result_type and (result == None or not isinstance(result, result_type)):
            raise TankError(
                "Unexpected type returned from 'hook_scene_operation' for operation '%s' - expected '%s' but returned '%s'"
    ...
    def do_snapshot(self, work_path, thumbnail, comment):
        ...
        if not os.path.exists(work_path):
            raise TankError(
                "Snapshot: Work file %s could not be found on disk!" % work_path
            )
        ...
        if not os.path.exists(snapshot_path):
            raise TankError(
                "Snapshot: Failed to copy work file from '%s' to '%s'"
    ...
    def restore_snapshot(self, current_path, snapshot_path, snapshot_current=True):
        ...
        if snapshot_current:
            ...
            if os.path.exists(current_path):
                try:
                    comment = (
                        "Automatic snapshot before restoring older snapshot '%s'"
        ...
        self._app.log_debug(
            "Snapshot Restore: Copying %s --> %s" % (snapshot_path, current_path)
        )
    ...
    def find_snapshot_history(self, file_path):
        ...
        files = self._app.tank.paths_from_template(
            self._snapshot_template, fields, ["version", "timestamp", "increment"]
        )
        if len(files) == 0:
            return history
        ...
        for file in files:
            ...
            if timestamp:
                details["datetime"] = datetime.strptime(
                    timestamp, Snapshot.TIMESTAMP_FMT
                )
    ...
    def show_snapshot_dlg(self):
        ...
        try:
            ...
            msg = (
                "Failed to get the current file path:\n\n"
                "%s\n\n"
    ...
    def show_snapshot_history_dlg(self):
        ...
        snapshot_history_form = self._app.engine.show_dialog(
            "Snapshot History", self._app, SnapshotHistoryForm, self._app, self
        )
    ...
    def _on_history_restore_snapshot(
        self, snapshot_history_form, current_path, snapshot_path
    ):
        """
        ...
        try:
            self.restore_snapshot(
                current_path, snapshot_path, snapshot_current=snapshot_current
            )
        except TankError as e:
    ...
    def _find_next_snapshot_increment(self, snapshot_fields):
        ...
        files = self._app.tank.paths_from_template(
            self._snapshot_template, snapshot_fields, ["timestamp", "increment"]
        )
    ...
    def _add_snapshot_thumbnail(self, snapshot_file_path, thumbnail):
        ...
        try:
            if not thumbnail.save(temp_path, "PNG"):
                raise TankError(
                    "Snapshot: Failed to save thumbnail to '%s'" % temp_path
                )
            ...
            self._app.log_debug(
                "Snapshot: Copying %s --> %s" % (temp_path, thumbnail_path)
            )
